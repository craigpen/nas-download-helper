...
const debugLog: any[] = [];
function dbg(level: string, msg: string, detail?: any) {
  const entry = {
...
async function getNasById(nasId: string) {
  const list = await getNasList();
  return list.find(n => n.id === nasId);
}

async function saveNasList(list: any[]) {
  return new Promise(resolve => chrome.storage.sync.set({ nasList: list }, resolve));
}

async function addNas(nas: any) {
  const list = await getNasList();
  list.push(nas);
  await saveNasList(list);
}

async function updateNas(nasId: string, updates: any) {
  const list = await getNasList();
...
async function deleteNas(nasId: string) {
  const list = await getNasList();
...
async function getStoredSid(nasId: string) {
...
async function storeSid(nasId: string, sid: string) {
...
async function removeSid(nasId: string) {
  return new Promise(resolve => {
    chrome.storage.local.get({ sids: {} }, r => {
      const sids = r.sids || {};
      delete sids[nasId];
...
async function synoFetch(label: string, url: string, options?: any, timeoutMs = 20000) {
...
async function synoLogin(s: any) {
...
async function getSid(nasId: string, s: any, force = false) {
  if (!force) {
...
async function synoCall(nasId: string, s: any, apiFn: (sid: string) => Promise<any>) {
...
async function synoAddMagnet(s: any, sid: string, magnetUrl: string) {
...
async function synoAddTorrent(s: any, sid: string, torrentUrl: string) {
  // Secondary validation check
...
async function testConnection(nasId: string, s: any) {
...
function decodeName(magnetUrl: string) {
  try {
...
async function sendMagnet(magnetUrl: string, nasId: string | null = null) {
  const list = await getNasList();
...
async function sendTorrent(torrentUrl: string, nasId: string | null = null) {
  const list = await getNasList();
  if (!nasId && list.length > 0) nasId = list[0].id;
...
async function listTasks(s: any, sid: string) {
...
async function taskAction(s: any, sid: string, action: string, ids: string[]) {
  const params = new URLSearchParams({
    api:     "SYNO.DownloadStation.Task",
    version: "1",
    method:  action,
  ...
  try {
    ...
    if (msg.type === "SEND_MAGNET") {
      sendMagnet(msg.url, msg.nasId)
        ...
        .catch((e: any) => sendResponse({ ok: false, error: e.message, log: [...debugLog] }));
      return true;
    }
    ...
    if (msg.type === "LIST_TASKS") {
      (async () => {
        ...
        synoCall(msg.nasId, s, sid => listTasks(s, sid))
          .then((tasks: any) => sendResponse({ ok: true, tasks }))
